...
def train_dqn_minimax(env, cfg, outdir, hidden=128):
    ...
    for ep in range(cfg.episodes):
        ...
        log.append({
            "episode": ep,
            "return_p1": float(ep_ret),
            "epsilon": float(epsilon_by_step(step, cfg.epsilon_start, cfg.epsilon_end, cfg.epsilon_decay_steps)),
            "td_loss": float(loss_sum / max(1, n_updates)),
