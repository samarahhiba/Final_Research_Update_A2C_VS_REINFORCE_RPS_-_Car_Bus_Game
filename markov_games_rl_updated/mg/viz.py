...
def plot_metric_compare(log_a, log_b, key: str, label_a, label_b, outpath: str | Path,
                        ma_window=50, title=None):
    """Compare two training logs on an arbitrary scalar metric key."""
    if not log_a or not log_b:
        return
    xa = [r.get(key, float('nan')) for r in log_a]
    xb = [r.get(key, float('nan')) for r in log_b]
    ma = moving_avg(xa, ma_window)
    mb = moving_avg(xb, ma_window)
    plt.figure()
    plt.plot(ma, label=label_a)
    plt.plot(mb, label=label_b)
    plt.legend()
    plt.title(title or f"{key} (moving avg)")
    plt.xlabel(f"episode (moving avg window={ma_window})")
    plt.ylabel(key)
    outpath = Path(outpath)
    outpath.parent.mkdir(parents=True, exist_ok=True)
